...
except Exception as e:  # pragma: no cover - user environment fix
    raise RuntimeError(
        "Missing Python dependencies. Activate your virtualenv and run: `pip install -r requirements.txt`"
    ) from e
...
def load_pickles():
    ...
    # If pickles are missing, don't crash the whole app on startup.
    missing = []
    for p, name in [
        (DF_PATH, "df.pkl"),
    ...
    if missing:
        # warn and leave TF-IDF features disabled; endpoints will return 500 with clear messages
        print(f"Warning: missing pickle files: {missing}. TF-IDF endpoints will be disabled until these are provided.")
        df = None
        indices_obj = None
        tfidf_matrix = None
        tfidf_obj = None
    ...
    try:
        ...
        if df is None or "title" not in df.columns:
            print("Warning: df.pkl must contain a DataFrame with a 'title' column; TF-IDF endpoints may fail.")
